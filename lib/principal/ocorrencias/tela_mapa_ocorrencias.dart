...
class _TelaMapaOcorrenciasState extends State<TelaMapaOcorrencias>
  ...
  String _perfilLogado = '';
  String _placaEquipeLogada = '';
  final Set<String> _ocorrenciasAlertadas = {};
  ...
  void initState() {
    ...
    _streamOcorrencias = FirebaseFirestore.instance
        .collection('Gerenciamento_ocorrencias')
        .snapshots();
    _streamEquipes =
        FirebaseFirestore.instance.collection('equipes').snapshots();
    _carregarAuxiliares();
  }
    ...
    if (user != null) {
      try {
        ...
        if (doc.exists && doc.data() != null) {
          ...
          setState(() {
            _perfilLogado =
                (dados['perfil'] ?? '').toString().toLowerCase().trim();
            if (_perfilLogado.contains('equipe') ||
                _perfilLogado.contains('tecnica') ||
                _perfilLogado.contains('técnica')) {
              _placaEquipeLogada =
                  (dados['placa_veiculo'] ?? '').toString().toUpperCase().trim();
            }
          });
        }
      } catch (e) {
        debugPrint("Erro ao buscar perfil no mapa: $e");
      }
    }
  }
  ...
  String _formatarDataHoraCompleta(dynamic t) {
    ...
    if (t is DateTime) {
      return DateFormat('dd/MM/yyyy HH:mm:ss').format(t);
    }
    return t.toString();
  }
  ...
  int _getStatusWeight(String statusRaw) {
    ...
    if (st.contains('aberto') ||
        ...
        st.contains('atribu')) return 1;
    if (st.contains('deslocamento')) return 2;
    if (st.contains('atendimento')) return 3;
    if (st.contains('aguardando baixa')) return 4;
    if (st.contains('conclu') || st.contains('finaliz')) return 5;
    return 6;
  }
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        if (data['nomeCompleto'] != null &&
            data['nomeCompleto'].toString().isNotEmpty) {
  ...
  Future<String> _gerarNumeroOcorrencia() async {
    ...
    DocumentReference contadorRef = FirebaseFirestore.instance
        .collection('contadores')
        .doc('ocorrencias_$anoAtual');
    return await FirebaseFirestore.instance
      ...
      if (snapshot.exists) {
        sequencia =
            ((snapshot.data() as Map<String, dynamic>)['atual'] ?? 0) + 1;
  ...
  Future<void> _carregarAuxiliares() async {
    try {
      ...
      for (var doc in f.docs) {
        ...
        String prio =
            (d['prioridade_da_falha'] ?? d['prioridade'] ?? 'BAIXA').toString();
        if (tipo.isNotEmpty) {
          prios[tipo] = prio;
          falhasLocal.add({
            'falha': tipo,
      ...
      for (var doc in s.docs) {
        var d = doc.data() as Map<String, dynamic>? ?? {};
        String idSemaforo =
            _formatarId((d['numero'] ?? d['id'] ?? doc.id).toString());
        dynamic geoData =
            d['georeferencia'] ?? d['coordenadas'] ?? d['localizacao'];
      ...
      List<String> origensLocal = o.docs
          .map((doc) =>
      ...
      List<String> materiaisLocal = m.docs
          .map((doc) =>
              ((doc.data() as Map<String, dynamic>)['nome'] ??
                      ...
                      (doc.data() as Map<String, dynamic>)['material'] ??
                      '')
                  .toString()
                  .toUpperCase())
          .where((mat) => mat.isNotEmpty)
          .toList();
  ...
  Future<Uint8List> _adicionarCarimboNaFoto(Uint8List imageBytes) async {
    try {
      ...
      canvas.drawRect(
          Rect.fromLTWH(0, image.height.toDouble() - 30,
              image.width.toDouble(), 30),
          paintRect);
      ...
      canvas.drawParagraph(
          paragraph, Offset(0, image.height.toDouble() - 20));
  ...
  String _calcularPrazo(Timestamp? dataAbertura, dynamic minutosPrazoStr) {
    ...
    DateTime limite =
        dataAbertura.toDate().add(Duration(minutes: minutos));
  ...
  bool _maisDe24h(Timestamp? dataAbertura) {
    ...
    return DateTime.now()
            .difference(dataAbertura.toDate())
            .inHours >=
        24;
  }
  ...
  bool _passouNoFiltro(Map<String, dynamic> data) {
    ...
    String prio =
        (_mapaPrioridades[data['tipo_da_falha']] ?? 'BAIXA').toLowerCase();
    bool prioOK = false;
  ...
  void _abrirOpcoesRota(String idSemaforoRaw) {
    String numSem = _formatarId(idSemaforoRaw);
    LatLng? c = _mapaCoordenadasSemaforos[numSem];
    if (c == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Coordenadas não encontradas para este semáforo no banco.')));
      return;
    }
  ...
  void _mostrarAlertaNovaOcorrencia(QueryDocumentSnapshot doc) {
    var data = doc.data() as Map<String, dynamic>;
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(
            ...
            side: const BorderSide(color: Colors.deepPurpleAccent, width: 3)),
        ...
        content: Column(
          ...
          children: [
            ...
            Text('🚦 Semáforo: ${data['semaforo']} - ${data['endereco']}',
                style: const TextStyle(
                    fontWeight: FontWeight.bold, fontSize: 14)),
            const SizedBox(height: 8),
            Text('Falha: ${data['tipo_da_falha']}',
                style: const TextStyle(
                    fontSize: 13,
        ...
        actions: [
          TextButton.icon(
            icon: const Icon(Icons.route, color: Colors.blueGrey),
            ...
            onPressed: () async {
              try {
                await FirebaseFirestore.instance
                    .collection('Gerenciamento_ocorrencias')
                    .doc(doc.id)
                    .update({'status': 'Em deslocamento'});
                if (ctx.mounted) Navigator.pop(ctx);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content:
                          Text('Status atualizado! Dirija com segurança.'),
                      backgroundColor: Colors.orange));
                }
              } catch (e) {
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text('Erro ao aceitar ocorrência.'),
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                image: DecorationImage(
                                    image: MemoryImage(fotosSelecionadas[index]),
                                    fit: BoxFit.cover),
                              ),
                            ),
                            Positioned(
                      ...
                      : () async {
                          if (defeitoConstatado && falhaMenuCtrl.text.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Preencha a falha encontrada!')));
                            return;
                          }
                          if (relatoCtrl.text.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Descreva a ação realizada nos detalhes!')));
                            return;
                          }
                          ...
                          try {
                            ...
                            for (var fotoBytes in fotosSelecionadas) {
                            ...
                            if (fotosBase64.isNotEmpty) {
                              updateData['fotos_finalizacao'] = fotosBase64;
                            }
                            ...
                            String semaforoInfo = _formatarId(data['semaforo']?.toString() ?? '');
                            String equipeNomes = data['integrantes_equipe'] ?? data['equipe_atrelada'] ?? data['equipe_responsavel'] ?? 'NÃO INFORMADA';
                            String defeitoRelatado = data['tipo_da_falha'] ?? 'NÃO INFORMADO';
                            String defeitoTexto = defeitoConstatado ? falhaMenuCtrl.text : 'DEFEITO NÃO CONSTATADO';
                            ...
                            if (kIsWeb) {
                              for (int i = 0; i < fotosSelecionadas.length; i++) {
                                arquivosParaCompartilhar.add(XFile.fromData(
                                    fotosSelecionadas[i],
                                    mimeType: 'image/png',
                                    name: 'foto_${docId}_${DateTime.now().millisecondsSinceEpoch}_$i.png'));
                              }
                            } else {
                              final tempDir = await getTemporaryDirectory();
                              for (int i = 0; i < fotosSelecionadas.length; i++) {
                                final file = File('${tempDir.path}/foto_${docId}_${DateTime.now().millisecondsSinceEpoch}_$i.png');
                                await file.writeAsBytes(fotosSelecionadas[i], flush: true);
  ...
  void _abrirModalCadastro(
      {String? docId, Map<String, dynamic>? dadosAtuais}) {
    ...
    if (falhaSel.isNotEmpty && !_opcoesFalhas.contains(falhaSel)) {
      _opcoesFalhas.add(falhaSel);
    }
    if (origemSel.isNotEmpty && !_opcoesOrigens.contains(origemSel)) {
      _opcoesOrigens.add(origemSel);
    }

    final semaforoMenuCtrl =
        TextEditingController(text: semaforoDropdownValue);
    final falhaMenuCtrl = TextEditingController(text: falhaSel);
    final origemMenuCtrl = TextEditingController(text: origemSel);
    final detalhesCtrl =
        TextEditingController(text: dadosAtuais?['detalhes'] ?? '');
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Autocomplete<String>(
                  initialValue:
                      TextEditingValue(text: semaforoDropdownValue),
                ...
                ElevatedButton(
                  ...
                  onPressed: estaSalvando
                      ? null
                      : () async {
                          if (semaforoMenuCtrl.text.isEmpty ||
                              !_opcoesSemaforos
                                  .contains(semaforoMenuCtrl.text)) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text(
                                        'Selecione um semáforo válido!')));
                            return;
                          }
                          ...
                          if (formKey.currentState!.validate()) {
                            setStateModal(() => estaSalvando = true);
                            try {
                              String semaforoFinal =
                                  semaforoMenuCtrl.text.split(' - ')[0];
                              ...
                              var semInfo = _semaforosAux.firstWhere(
                                  (s) => s['id'] == semaforoFinal,
                                  orElse: () => <String, dynamic>{});
                              var falhaInfo = _falhasAux.firstWhere(
                              ...
                              if (docId == null) {
                                String numOcorrencia =
                                    await _gerarNumeroOcorrencia();
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // ── Seção de fotos da equipe destacada ──
                  ...
                  onChanged: (v) {
                    setStateModal(() {
                      ...
                      if (!defeitoConstatado) {
                        ...
                        acaoCtrl.text = dados['relato_equipe'] ?? '';
                      }
                    });
                  },
                ),
                ...
                TextFormField(
                  ...
                  inputFormatters: [UpperCaseTextFormatter()],
                  decoration: const InputDecoration(
                      labelText: 'Ação Técnica Confirmada *',
                      border: OutlineInputBorder()),
                ),
                const SizedBox(height: 16),
                if (defeitoConstatado) ...[
                  const Text('MATERIAIS UTILIZADOS (Opcional)',
                      style: TextStyle(
                          fontWeight: FontWeight.bold,
                ...
                ElevatedButton(
                  ...
                  onPressed: estaSalvando
                      ...
                      : () async {
                          ...
                          try {
                            List<String> novasFotosBase64 = [];
                            for (var fotoBytes in fotosSelecionadas) {
                              novasFotosBase64.add(base64Encode(fotoBytes));
                            }
                            ...
                            if (dados['fotos_finalizacao'] != null) {
                              fotosFinais.addAll(dados['fotos_finalizacao']);
                            }
                            if (novasFotosBase64.isNotEmpty) {
                              fotosFinais.addAll(novasFotosBase64);
                            }
                  ...
                  child: estaSalvando
                      ? const SizedBox(
                          ...
                          child: CircularProgressIndicator(
                              color: Colors.white, strokeWidth: 2))
                      : const Text('Concluir Atendimento e Dar Baixa',
                          style: TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.bold)),
                ),
            ...
            children: [
              ...
              if (equipesAtivas.isEmpty)
                const Padding(
                    ...
                    child: Text(
                        'Nenhuma equipe ATIVA encontrada no momento.'))
              else
                ...equipesAtivas.map((eq) {
                  ...
                  String nomeLider =
                      ints.split(',').first.trim().toUpperCase();
  ...
  void _registrarChegada(String docId) async {
    bool? conf = await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Registrar Chegada',
  ...
  void _abrirDetalhes(String docId, Map<String, dynamic> data) {
    ...
    if (st.contains('aguardando baixa')) corBase = Colors.teal;
          ...
          children: [
            ...
            if (!isEquipeLogada || ehDessaEquipe) ...[
              if (st.contains('atendimento')) ...[
                ...
                ElevatedButton(
                  ...
                  child: const Text('Deslocar (Aceitar Ocorrência)',
                      style: TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold)),
                ),
                ...
                ElevatedButton(
                  ...
                  child: const Text('Atribuir Equipe Responsável',
                      style: TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold)),
                ),
              ...
              const Center(
                  child: Text(
                      'Esta ocorrência está atrelada a outra equipe.',
                      style: TextStyle(
                  ...
                  onPressed: () {
                    ...
                    showDialog(
                      ...
                      builder: (ctx) => AlertDialog(
                        ...
                        title: Text('Histórico do $numSem',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold,
                                color: Colors.blueGrey,
                                fontSize: 18)),
                                    ...
                                    children: [
                                      ...
                                      Text(
                                          ...
                                          style: const TextStyle(
                                              ...
                                              color: Colors.black87)),
                                      Text(
                                          'Equipe Resp.: ${d['equipe_atrelada'] ?? d['equipe_responsavel'] ?? '-'}',
                                          style: const TextStyle(
                                              fontSize: 12,
                                              color: Colors.black87)),
                                      Text(
                                          'Falha relatada: ${d['tipo_da_falha'] ?? '-'}',
                                          style: const TextStyle(
                                              fontSize: 12,
                                              color: Colors.black87)),
                                      RichText(
            ...
            ElevatedButton.icon(
              ...
              onPressed: () {
                ...
                if (numSem.isNotEmpty) {
                  numSem = numSem.padLeft(3, '0');
                }
                Navigator.pop(context);
                Navigator.push(
                    context,
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const Divider(),
                    _buildInfoRow('Usuário Abertura',
                        data['usuario_abertura'] ?? data['usuario']),
                    _buildInfoRow('Equipe Resp.',
                        data['equipe_atrelada'] ??
                            data['equipe_responsavel']),
                    ...
                    if (data['materiais_utilizados'] != null &&
                        (data['materiais_utilizados'] as List)
                            .isNotEmpty) ...[
                      const Divider(),
                    ...
                    if (data['fotos_finalizacao'] != null &&
                        (data['fotos_finalizacao'] as List)
                            .isNotEmpty) ...[
                      const Divider(),
                      const Text('Fotos da Finalização:',
                          style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF2c3e50),
                              fontSize: 13)),
  ...
  Widget _buildSidebarPendentes() {
    ...
    if (pendentes.isEmpty) {
      return const Padding(
        ...
        child: Center(
          child: Text('Nenhuma ocorrência aguardando.',
  ...
  Widget _buildSidebarEquipes() {
    ...
    if (ativas.isEmpty) {
      return const Padding(
        padding: EdgeInsets.all(16),
        child: Center(
          child: Text('Nenhuma equipe em campo no momento.',
              style: TextStyle(color: Colors.white70, fontSize: 12)),
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        String tipoVeiculo =
            (eq['tipo_veiculo'] ?? eq['tipo'] ?? '').toString().toUpperCase();
        ...
        final tarefas = _todasOcorrencias.where((oc) {
          ...
          if (isConcluido) {
            if (d['data_de_finalizacao'] != null) {
              DateTime dtFim =
                  (d['data_de_finalizacao'] as Timestamp).toDate();
              if (DateTime.now().difference(dtFim).inHours > 24) {
                return false;
              }
            }
          }
        ...
        tarefas.sort((a, b) {
          ...
          return getOrder(a.data() as Map<String, dynamic>)
              .compareTo(getOrder(b.data() as Map<String, dynamic>));
        });
                ...
                child: tarefas.isEmpty
                    ...
                    : Column(
                        ...
                        children: tarefas.map((tDoc) {
                          ...
                          String empresaSemaforo =
                              (t['empresa_semaforo'] ?? '').toString();
                          if (empresaSemaforo.isEmpty) {
                            String numSemAux = _formatarId(
                                t['semaforo']?.toString() ?? '');
                            var semInfo = _semaforosAux.firstWhere(
                                (s) => s['id'] == numSemAux,
                                orElse: () => <String, dynamic>{});
                            empresaSemaforo =
                                (semInfo['empresa'] ?? '---').toString();
                          }
  ...
  Widget _buildSidebarMinhaEquipe(
      List<QueryDocumentSnapshot> minhasOcorrencias) {
    ...
    return ListView.builder(
      ...
      itemCount: minhasOcorrencias.length,
      itemBuilder: (ctx, i) {
        var doc = minhasOcorrencias[i];
        var d = doc.data() as Map<String, dynamic>;
        String st = (d['status'] ?? '').toString().toLowerCase();
        String stDisplay = st.toUpperCase();
        Color cor = _corStatusReal(st);

        return Card(
          margin: const EdgeInsets.only(bottom: 12),
          shape: Border(left: BorderSide(color: cor, width: 5)),
          child: Padding(
            padding: const EdgeInsets.all(12),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text('🚦 ${d['semaforo']} - ${d['endereco']}',
                    style: const TextStyle(
                        fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 4),
                    ...
                    Expanded(
                      child: ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.orange),
                        onPressed: () =>
                            _abrirModalRelatoWhatsApp(doc.id, d),
                        child: const Text('Pré-Concluir',
                            style: TextStyle(
                                fontSize: 11,
                                color: Colors.white,
                                fontWeight: FontWeight.bold)),
                ...
                builder: (context, snapshotEq) {
                            ...
                            onTap: () {
                              if (listaDocs.length == 1) {
                                ...
                                showDialog(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    title: Text(
                                        'Ocorrências no Semáforo $numSem',
                                        style: const TextStyle(
                                            fontWeight: FontWeight.bold,
                                            color: Colors.blueGrey)),
                                    content: SizedBox(
                  ...
                  if (isEquipeLogada) {
                    var novasAtribuidas = ocorrenciasParaMapa.where((doc) {
                      ...
                      return st.contains('atribu');
                    }).toList();

                    for (var nova in novasAtribuidas) {
                      if (!_ocorrenciasAlertadas.contains(nova.id)) {
                                    ...
                                    if (!isEquipeLogada)
                                      Positioned(
                                        ...
                                        child: AnimatedContainer(
                                          ...
                                          width: _filtrosVisiveis
                                              ? 180
                                              : 0,
                                          height: _filtrosVisiveis
                                              ? 460
                                              : 0,
                                          padding: _filtrosVisiveis
                              ...
                              sidebarWidget = Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                        child: _buildSidebarMinhaEquipe(
                                            ocorrenciasParaMapa)),
                                  ],
                                ),
                              );
