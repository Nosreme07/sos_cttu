...
class _TelaBuscaState extends State<TelaBusca> {
  ...
  final List<String> _opcoesFiltro = [
    'Todos', 'Semáforo', 'Endereço', 'Bairro', 'Subárea', 'Empresa', 'Rota'
  ];
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('Semáforo $idFormatado', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF2f3b4c))),
                        Text(data['endereco'] ?? '-', style: const TextStyle(fontSize: 13, color: Colors.black87)),
                      ],
                    ),
                  ),
                  IconButton(
              ...
              ElevatedButton.icon(
                ...
                label: const Text('Nova Ocorrência', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                onPressed: () {
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Abertura de nova ocorrência será ativada no próximo módulo!')));
                },
              ),
              const SizedBox(height: 8),
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Não foi possível abrir o Waze')));
                  }
                },
              ),
              const SizedBox(height: 16),
      ...
      pw.MultiPage(
        ...
        footer: (pw.Context context) {
          return pw.Container(
            ...
            child: pw.Text('Gerado pelo SOS em: $dataHora', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey700), textAlign: pw.TextAlign.center),
          );
        },
          ...
          for (var grupo in _gruposFormulario) {
            ...
            conteudo.add(
              pw.TableHelper.fromTextArray(
                context: context, cellAlignment: pw.Alignment.centerLeft,
                headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.white),
                headerDecoration: const pw.BoxDecoration(color: PdfColors.blueGrey600),
                data: <List<String>>[ <String>['Campo', 'Informação'], ...tabelaGrupo ],
              )
            );
  ...
  Future<void> _exportarCsvIndividual(Map<String, dynamic> data, String numeroFormatado) async {
    ...
    for (var grupo in _gruposFormulario) {
      ...
      for (var campo in grupo['campos']) {
        String valor = (data[campo['key']] ?? '').toString();
        if (valor.isNotEmpty) rows.add([campo['label'].toString().replaceAll(' *', ''), valor]);
      }
      rows.add([]);
    }
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              docs.sort((a, b) {
                int numA = int.tryParse(((a.data() as Map)['id'] ?? '').toString().replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
                int numB = int.tryParse(((b.data() as Map)['id'] ?? '').toString().replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
              ...
              List<QueryDocumentSnapshot> semaforosFiltrados = docs.where((doc) {
                ...
                switch (_filtroSelecionado) {
                  case 'Semáforo': return contemTexto(idOriginal) || contemTexto(idFormatado);
                  case 'Endereço': return contemTexto(endereco);
                  case 'Bairro': return contemTexto(bairro);
                  case 'Subárea': return contemTexto(subarea);
                  case 'Empresa': return contemTexto(empresa);
                  case 'Rota': return contemTexto(rota);
                  case 'Todos':
                  default:
                    return contemTexto(idOriginal) || contemTexto(idFormatado) || contemTexto(endereco) || 
                           contemTexto(bairro) || contemTexto(subarea) || contemTexto(empresa) || contemTexto(rota);
                }
              }).toList();
                  ...
                  marcadores.add(
                    Marker(
                      ...
                      child: GestureDetector(
                        onTap: () => _abrirDetalhesResumidos(data), // AQUI CHAMA O NOVO MODAL
                        child: iconeMarcador,
                      ),
                    )
                  );
                              ...
                              Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton<String>(
                                    ...
                                    items: _opcoesFiltro.map((String f) => DropdownMenuItem(value: f, child: Text(f))).toList(),
                                    onChanged: (novoValor) {
                                      setState(() {
                  ...
                  Expanded(
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        Widget mapaWidget = Container(
                          ...
                          child: ClipRRect(
                            ...
                            child: FlutterMap(
                              ...
                              children: [
                                TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png', userAgentPackageName: 'com.seusistema.sos'),
                          ...
                          child: Column(
                            children: [
                              Container(
                                ...
                                child: Text('RESULTADO - ${semaforosFiltrados.length}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16), textAlign: TextAlign.center),
                              ),
                              Expanded(
                                    ...
                                    return Card(
                                      ...
                                      child: ListTile(
                                        ...
                                        onTap: () {
                                          if (coords != null) {
                                            _focarNoMapa(coords);
                                            _abrirDetalhesResumidos(data); // AQUI CHAMA O NOVO MODAL
                                          } else {
                        ...
                        if (isDesktop) {
                          return Row(children: [Expanded(flex: 3, child: mapaWidget), Expanded(flex: 1, child: listaWidget)]);
                        } else {
                          return Column(children: [Expanded(flex: 3, child: mapaWidget), Expanded(flex: 2, child: listaWidget)]);
                        }
                      },
                    ),
                  ),
